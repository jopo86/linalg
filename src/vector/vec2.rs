...
impl<T: Float, const C: usize> Mul<Mat<T, 1, C>> for Vec2<T> {
    type Output = Mat<T, 2, C>;
...
mod tests {
    ...
    fn mul_mat() {
        assert_eq!(
            Vec2::new(1.0, 2.0) * Mat::new(&[[4.0, 3.0, 2.0, 1.0]]),
            Mat::new(&[[4.0, 3.0, 2.0, 1.0], [8.0, 6.0, 4.0, 2.0],])
        )
    }
}
